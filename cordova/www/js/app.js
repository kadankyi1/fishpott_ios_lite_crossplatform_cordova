...
function checkSignupPersonalForm(){
    ...
    var this_agreement = document.getElementById('eula_agreement_pa_siginin');

    if(this_country.trim() == "" || this_full_name.trim() == "" || this_pot_name.trim() == ""  || 
        this_password.trim() == "" || this_dob.trim() == "" || this_dob.trim() == "" || 
        this_gender.trim() == "" || this_repeat_password.trim() == "" || !this_agreement.checked) {

         if(!this_agreement.checked){
        ...
        this_pot_name.trim() != ""  && this_password.trim() != "" && this_gender.trim() != ""
         && this_phone.trim() != "" && this_dob.trim() != "" && this_gender.trim() != "" && this_agreement.checked) {
...
function checkSignupBusinessForm(){
    ...
    var this_agreement = document.getElementById('eula_agreement_ba_siginin');
...
function login(){
    ...
    var this_agreement = document.getElementById('eula_agreement_login');
    ...
    }  else if(!this_agreement.checked){

          showToast("You have to agree to the End-User License Agreement");

    }  else if(this_phone.trim() != "" && this_password.trim() != "" && this_agreement.checked){
...
function copyPottAddress(){
      if(this_pottname == "???"){

        pottname_for_url = my_pot_name;

      } else {

        pottname_for_url = this_pottname;

      }
      text = "https://fishpott.com/web/in/pott_ref.php?pottname=" + pottname_for_url;
      console.log("text : " + text);
    var this_message = "Copied";
    showToast("Pott Address Url Copied.");
    if (window.clipboardData && window.clipboardData.setData) {
        // IE specific code path to prevent textarea being shown while dialog is visible.
        this_message = "Copied";
        return clipboardData.setData("Text", text); 
                ...
                for (var i = news_show_toggle; i  < total_news_num; i++) {
                        ...
                        if(i == 0){
                            ...
                            if(news_type == "1"){
                                ...
                                $('#'+holder_id).append($('<div class="card demo-facebook-card" style="margin-right: 0; margin-left: 0;"><div class="card-header"><a href="#" class="link" style="float: right;"><i class="icon f7-icons ios-only" style="font-size: 20px;color: #ff6300">heart_fill</i></a><div class="demo-facebook-avatar popup-open" data-popup=".pott" data-pottname = "'+ news_maker_pottname +'"  onclick="getPottInfo(this);"  style="background-image: url(' + news_maker_pro_pic + '); background-size: cover; position: relative; width: 45px; padding-top: 45px; border-radius: 50%; margin-right : 5px;"></div><div class="demo-facebook-name popup-open" data-popup=".pott" data-pottname = "'+ news_maker_pottname +'"  style="font-weight : bolder;"   onclick="getPottInfo(this);" >' + news_maker_full_name + '</div><div class="demo-facebook-date" style="font-style:italic; color:green;">@' + news_maker_pottname + '</div><div class="demo-facebook-date">' + news_date + '</div></div></div>'));
                            
                            } else {

                                $('#'+holder_id).append($('<div class="card demo-facebook-card" style="margin-right: 0; margin-left: 0;"><div class="card-header"><a href="#" class="link" style="float: right;"><img src="img/dislike_fill.svg" width="24px" height="24px"></a><div class="demo-facebook-avatar popup-open" data-popup=".pott" data-pottname = "'+ news_maker_pottname +'"  onclick="getPottInfo(this);"  style="background-image: url(' + news_maker_pro_pic + '); background-size: cover; position: relative; width: 45px; padding-top: 45px; border-radius: 50%; margin-right : 5px;"></div><div class="demo-facebook-name popup-open" data-popup=".pott" data-pottname = "'+ news_maker_pottname +'"  onclick="getPottInfo(this);" style="font-weight : bolder;" >' + news_maker_full_name + '</div><div class="demo-facebook-date" style="font-style:italic; color:green;">@' + news_maker_pottname + '</div><div class="demo-facebook-date">' + news_date + '</div></div></div>'));
...
function BlockUnblockPott(x){

    block_status = x.getAttribute('data-status');

    if(block_status == "0"){

        app.dialog.preloader('Blocking...');

      } else {
        app.dialog.preloader('Un-Blocking...');
      }

    var url_real = CONFIG + 'inc/android_block_unblock_pott.php';
     ...
     var loginData = {
        ...
        "blocked_pottname" : pottNameGlobal
    };          
        ...
        console.log("response : " + response);
            ...
            if(response == "1"){
              ...
              if(block_status == "1"){
                  ...
                  document.getElementById("block_list_item_inner").innerHTML = "Block Pott";
                  app.dialog.alert('Pott is now unblocked. You will see posts and receive messages from this pott after reload', 'UNBLOCKED');

              } else {

                  x.setAttribute('data-status', "1");
                  document.getElementById("block_list_item_inner").innerHTML = "Unblock Pott";
                  app.dialog.alert('Pott is now blocked. You will no longer see posts or receive messages from this pott after reload', 'BLOCKED');
 ...
 function callbackUnblock(){

    block_btn = document.getElementById("block_list_item");
    BlockUnblockPott(block_btn);
...
function getPottInfo(x){
    ...
    if(this_pottname == my_pot_name){
      ...
      document.getElementById("block_list_item").style.display = "none";
      ...
      document.getElementById("block_list_item").style.display = "";
     ...
     if(this_pottname == '???'){
      ...
      if(this_pottname.trim() != ""){
        ...
        app.request({
          ...
          data: loginData,
          success: function(response){
            console.log("pott bios response");
            app.preloader.hide();
            ...
            if(total_bios_data > 0 && biosResponse["pott_info"][0]["full_name"] != "" && 
                ...
                this_blocked_status = biosResponse["pott_info"][0]["blocked_status"];
                curr_pott_sys_id = this_pott_investor_id;

                this_linkup_btn = document.getElementById('linkup_btn_main');
                this_linkup_btn.setAttribute("data-status", this_pott_our_link);

                  this_block_list_item = document.getElementById('block_list_item');
                pottNameGlobal = this_pottname_real;

                if(this_blocked_status == "1"){

                  this_block_list_item.setAttribute("data-status", "1");
                  document.getElementById("block_list_item_inner").innerHTML = "Unblock Pott";
                  $('#other_pott_close_main').click();
                  app.dialog.confirm('You have blocked this pott. Unblock?', 'BLOCKED', callbackUnblock, callbackCancel);
                  return;
                } else {

                  document.getElementById("block_list_item_inner").innerHTML = "Block Pott";
                  this_block_list_item.setAttribute("data-status", "0");
